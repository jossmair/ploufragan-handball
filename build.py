...
def heading_video(source, poster, marker, css_class):
    return (
        ...
        f'<video data-heading-video {marker} muted playsinline preload="metadata" '
...
def product_categories(product):
    primary = product_category(product)
    categories = [primary]
    name = product["name"].upper()
    child_to_adult = primary == "homme" and not any(
        label in name for label in ("SWEAT A CAPUCHE ADULTE", "T SHIRT COTON HOMME")
    )
    if child_to_adult:
        categories.append("enfant")
    return categories
...
pages["resultats"]=page("resultats","Résultats et championnats",heading("RÉSULTATS<br><em>& CHAMPIONNATS</em>","Résultats","Les données FFHandball sont actualisées plusieurs fois par jour.",css_class="animated-heading results-heading",extra=results_heading_media)+f'''<section class="container section after-heading">{results_filter}<div class="section-heading"><div><p class="eyebrow">{scores_eyebrow}</p><h2>LES <em>SCORES</em></h2></div>{sync_source}</div><div class="matches-grid" data-results-scores>{''.join(match_card(m, pending_score=not m["played"]) for m in score_matches)}</div><div class="section-heading spaced"><h2>PROCHAINS <em>MATCHS</em></h2></div><div class="matches-grid" data-results-upcoming>{''.join(match_card(m) for m in next_round_matches(upcoming))}</div><div class="section-heading spaced"><div><p class="eyebrow">9 ÉQUIPES ENGAGÉES</p><h2>SUIVRE LES <em>CHAMPIONNATS</em></h2></div></div><div class="competitions-grid">{competition_cards}</div></section>''')
